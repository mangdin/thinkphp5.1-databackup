...
class Backup
{
    ...
    public function fileList()
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\\w+-\\d{8,8}-\\d{6,6}-\\d+\\.sql(?:\\.gz)?$/', $name)) {
//                $name = sscanf($name, '%4s%2s%2s-%2s%2s%2s-%d');
                $name_arr = explode('-',$name);
                $length = strlen($name_arr['0']);
                $name = sscanf($name,"%".$length."s-%4s%2s%2s-%2s%2s%2s-%d");
//                $name = sscanf()
                $date = "{$name[1]}-{$name[2]}-{$name[3]}";
                $time = "{$name[4]}:{$name[5]}:{$name[6]}";
                $part = $name[7];
        ...
        switch ($type) {
            ...
            case 'timeverif':
                ...
                foreach ($files as $name) {
                    ...
                    $name_arr = explode('-',$basename);
                    $length = strlen($name_arr['0']);
                    $match = sscanf($basename, "%".$length."s-%4s%2s%2s-%2s%2s%2s-%d");
                    $gz = preg_match('/^\\w+-\\d{8,8}-\\d{6,6}-\\d+\\.sql.gz$/', $basename);
    ...
    {
        if(file_exists($this->config['path'].$name)){
            unlink($this->config['path'].$name);
            return json(['msg'=>'删除成功','code'=>200]);;
        }else{
            return json(['msg'=>'文件不存在','code'=>500]);
        }
//        if ($time) {
...
//        }
    ...
    public function import($start)
    {
        ...
        if ($this->config['compress']) {
            ...
            $size = filesize($this->config['path'].$this->file['name']);
            $gz = fopen($this->config['path'].$this->file['name'], 'r');
        }
        $sql = '';
        if ($start) {
    ...
    {
        $this->file = ['name' => $table.'-'.date('Ymd-His'), 'part' => 1];
        $db = self::connect();
        // 备份表结构
        if (0 == $start) {
    ...
    public function sql_all(){
        ...
        $filename=$dbname."-".date('Ymd-His')."-1.sql";
        $fp = fopen($this->config['path'].$filename,'w');
        fputs($fp,$mysql);
        fclose($fp);
        return 0;
    }
